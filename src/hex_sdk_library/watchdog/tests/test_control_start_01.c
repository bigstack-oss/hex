...
main() 
{
    ...
    HEX_TEST_FATAL(HexDaemonStart(NULL, "./daemontest", 0) == -1);

    // Should return true and start program
    HEX_TEST_FATAL(HexDaemonStart(PROGRAM, "./daemontest", 0) == 0);
    WaitForStart();
    ...
    HEX_TEST_FATAL(pid > 0);
    HEX_TEST_FATAL(access(SIGHUP_TEST_FILE, F_OK) != 0);
    HEX_TEST_FATAL(HexDaemonStart(PROGRAM, "./daemontest", HEX_DAEMON_NO_SIGHUP) == 0);
    ...
    // Should return true because program was restarted (args have changed)
    // Pid should change, SIGHUP signal should not be sent
    pid = HexPidFileRead(PID_FILE);
    HEX_TEST_FATAL(pid > 0);
    HEX_TEST_FATAL(access(SIGHUP_TEST_FILE, F_OK) != 0);
    HEX_TEST_FATAL(HexDaemonStart(PROGRAM, "./daemontest args", 0) == 0);
    WaitForPidChange(pid);
    HEX_TEST_FATAL(access(SIGHUP_TEST_FILE, F_OK) != 0);

    // Cmd line file should contain "./daemontest args"
    HEX_TEST_FATAL(HexSystemF(0, "grep '^./daemontest args$' %s", CMD_LINE_FILE) == 0);
